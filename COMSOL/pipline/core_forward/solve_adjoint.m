function [lambda, ok, lambda_gauss] = solve_adjoint(model, voxel, p, f_adj, source_pos, Ms)
...
%% 3. Zero background field (adjoint_mode=0 + 显式 Eb=0)
% ★ H029 迁移: 验证管线确认仅靠 adjoint_mode=0 不足以归零背景场
%   需显式设 BackgroundField.Eb=[0,0,0] 确保伴随求解不受背景场污染
...
% B03 修正 2026-06-27: lambda = lambda_raw (no conj!) — 适用于 Born 体积电流路径
% ★ FD 标定 2026-07-26: 双源路径用 conj(lambda_raw)（表面源的共轭约定与体积源不同）
if ok
    try
        [lambda_raw, ~] = read_field(model, voxel.pos(inner, :));
        if use_dual_source
            lambda = conj(lambda_raw);  % ★ 双源路径需要共轭
        else
            lambda = lambda_raw;  % Born 路径: no conj (B03 fix)
        end
    ...
    if ok && nargout >= 3 && ~isempty(voxel.gauss_pos)
        try
            ...
            if use_dual_source
                lambda_gauss = conj(lambda_raw_g);  % ★ 双源路径共轭
            else
...
try
    ...
    % ★ H029 迁移: 恢复背景场 Eb=[0,0,1]（验证管线确认的正演模式背景场）
