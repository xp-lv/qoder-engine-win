# pipline3 伴随法验证管线 (Adjoint Verification Pipeline)
...
**模型文件**（位于管线根目录）:
```
COMSOL/pipline3/2layer.mph   ← 极简 2 层球模型（散射体 + PML）
```
...
│   ├── solve_adjoint.m       ← 伴随求解（体积/表面双路径自动判断）[核心]
...
│
├── core_jhyp/                ← 伴随源构建层
│   ├── build_adjoint_source_fullmaxwell.m ← 表面双源 Stratton-Chu 伴随源（管线2 遗留）
│   └── build_adjoint_source_volume.m    ← 体积等效源伴随源（管线3 主路径）[算法核心]
│
├── core_adjoint/             ← 梯度与代价层
│   ├── compute_gradient.m    ← 精确梯度 g=g_direct+g_indirect（原理 §7.4，bilinear/hermitian 模式）[算法核心]
│   └── compute_cost.m        ← 代价函数 F（定义 A：归一化残差）
│
├── experiment/               ← 验证脚本（16 个文件）
│   ├── verify_per_voxel_volume.m ← ★ 逐体素方向验证（体积路径，sign/cosθ/CV）[主验证]
...
```powershell
cd "d:\ZJU\PROJECT\2026-07-02-qoder-engine\COMSOL\pipline3"
& "D:\LenovoSoftstore\Install\MATLAB\bin\matlab.exe" -batch "addpath('config','utils','core_forward','core_jobs','core_jhyp','core_adjoint','experiment'); addpath('D:\LenovoSoftstore\Install\COMSOL62\Multiphysics\mli'); verify_per_voxel_volume(30);"
```
...
%% 真值 eps_r=5 → J_obs（Stratton-Chu，原理 §8.2）
...
%% 初值 eps_r=3 → 正演 E + J_hyp（Born FT，原理 §8.2）
...
%% 伴随求解（不传 Ms → 自动走体积路径）
source_pos_vol = voxel.pos(find(inner), :);
[lambda, ok, lambda_gauss] = solve_adjoint(model, voxel, p, f_adj, source_pos_vol);

%% 完整梯度 g = g_direct + g_indirect（原理 §7.4）
%   g_direct   = +2·ω·ε₀·dV·Im[conj(E)·S] / F_obs
%   g_indirect = -k₀²·dV·Re[conj(E)·λ]
omega_eps0 = p.omega(1) * p.eps0; k0_sq = p.k0^2;
inner_idx = find(inner);
g = zeros(sum(inner), 1);
for vi = 1:sum(inner)
    dV_v = voxel.dV(inner_idx(vi));
    g_direct = +2 * omega_eps0 * dV_v * imag(sum(conj(E_total(vi,:)) .* S_field(vi,:))) / F_obs;
    g_indirect = -k0_sq * dV_v * real(sum(conj(E_total(vi,:)) .* lambda(vi,:)));
    g(vi) = g_direct + g_indirect;
end
