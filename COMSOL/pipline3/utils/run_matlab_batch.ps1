...
.SYNOPSIS
    ...
    (pipeline maintenance H035-P0, consolidated by pipeline maintainer)

.DESCRIPTION
    H035 discovered that Start-Process -RedirectStandardOutput FAILS to capture
    MATLAB -batch stdout in this environment: the process exits in ~25s with
    exit code 0 but stdout.log contains only the marker line (~57 bytes) and
    MATLAB application output is swallowed (0 bytes). The .mat output file is
    ...
    MAINTENANCE HISTORY:
      H035-P0 : Created canonical launcher; replaced Start-Process -Redirect
                with .bat-wrapper cmd.exe full-stream redirection; added
                mtime guard for silent-fail detection.
...
.USAGE
    powershell -ExecutionPolicy Bypass -File run_matlab_batch.ps1 `
        -BatchCommand "addpath('config','experiment'); run_inversion_complex();" `
        -LogFile "D:\path\to\stdout.log" `
        -WorkDir "d:\ZJU\PROJECT\2026-07-02-qoder-engine\COMSOL\pipline_adjoint" `
        -OutputFile "d:\ZJU\PROJECT\2026-07-02-qoder-engine\COMSOL\pipline_adjoint\data\results\inversion_complex_result.mat"
