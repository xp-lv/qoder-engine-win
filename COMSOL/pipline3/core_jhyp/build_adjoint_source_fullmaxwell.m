...
% 标量系数
% ★ 管线3 状态：管线3 实际走体积等效源路径（build_adjoint_source_volume），
%   本文件（表面双源路径）为管线2 遗留代码。solve_adjoint.m 中 use_dual_source
%   现已由 Ms 参数自动判断（非空时走双源路径），不传 Ms 时此文件不参与计算。
% coeff_base = 0.5i·ω·ε₀：表面双源路径的标量系数（经 FD 标定确认 sign 正确）
coeff_base = 0.5i * omega * eps0;
