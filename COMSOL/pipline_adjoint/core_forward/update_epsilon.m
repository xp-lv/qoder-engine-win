...
%
%   原理: COMSOL GUI 中设置 ε_r = int1(x,y,z) + i*int2(x,y,z)，
%   此处通过更新 int1/int2 数据表传递新的 ε_r 分布。
%
%   pipline_adjoint 模型节点 tag:
%     int1 = 实部 (eps_init.txt)
%     int2 = 虚部 (eps_init_im.txt)
%     表达式: int1(x,y,z) + i*int2(x,y,z)
...
try
    ...
    model.component('comp1').func('int1').importData(csv_path);
    delete(csv_path);
    
    fprintf('  OK int1 (Re) 已更新\n');
    ...
    model.component('comp1').func('int2').importData(csv_path_im);
    delete(csv_path_im);
    
    if has_imag
        fprintf('  OK int2 (Im) 已更新\n');
    end
...
if complex_epsr_configured
    try
        ...
        if ~isempty(val_check) && (contains(val_check, 'int1') || contains(val_check, 'int2'))
    ...
    for i = 1:length(fc)
        ...
        for pi = 1:length(prop_names)
            ...
            try
                ...
                val = char(val);
                if ~isempty(val) && (contains(val, 'int1') || contains(val, 'int2'))
...
if set_ok
    ...
    fprintf('  [WARN] 无法自动设置复数 ε_r. 请在 COMSOL GUI 中手动设 ε_r = int1 + i*int2\n');
end
end
