...
params.output_path = 'D:/ZJU/PROJECT/2026-07-02-qoder-engine/engine/sdk/forward_pipeline/.e2e_runs/hollow_sphere/compare_run/B/B_hollow_scatter.mat';
...
fprintf('[B_hollow] voxel_positions: %d x %d\n', size(params.voxel_positions));

result = simple_forward_solve(params);

fprintf('\n[B_hollow] === Result ===\n');
fprintf('[B_hollow] status: %s\n', result.status);
if strcmp(result.status, 'error')
    fprintf('[B_hollow] error: %s\n', result.error_msg);
end
if isfield(result, 'scattered_field')
    Es = result.scattered_field.E_s;
    fprintf('[B_hollow] ||E_s|| = %.4e\n', sqrt(sum(abs(Es(:)).^2)));
end

fid = fopen('D:/ZJU/PROJECT/2026-07-02-qoder-engine/engine/sdk/forward_pipeline/.e2e_runs/hollow_sphere/compare_run/B/B_hollow_scatter.flag', 'w');
fprintf(fid, '%d\n', strcmp(result.status, 'success'));
fclose(fid);

clearvars m result;
fprintf('[B_hollow] DONE\n');
