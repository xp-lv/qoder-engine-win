"""探查 livelink_model.mph 的 emw 物理·场结构，找到 epsilonr 控制点。"""
import subprocess
import sys
import io
...
WORK = PROJECT_ROOT / "engine/sdk/forward_pipeline/.e2e_runs/hollow_sphere/probe_run"
...
matlab_file = WORK / "probe_emw.m"
matlab_file.write_text(script, encoding="utf-8", errors="replace")

print(f"Probing livelink_model.mph...")

cmd = [r"D:\LenovoSoftstore\Install\MATLAB\bin\matlab.exe",
       "-batch", f"run('{str(matlab_file).replace(chr(92), '/')}')"]
