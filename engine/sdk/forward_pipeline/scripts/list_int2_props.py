"""列出 Interpolation function 的所有可用属性名。
通过 probe 一个新创建的 Interpolation function 来发现。"""
import subprocess
import sys
import io
...
WORK = PROJECT_ROOT / "engine/sdk/forward_pipeline/.e2e_runs/hollow_sphere/list_props"
...
try
    ...
    fid = fopen('<<TEST_CSV_SPACE>>', 'w');
    fwrite(fid, csv_content);
    fclose(fid);
    m.func('int2_new').set('tablename', 'data');
    m.func('int2_new').set('data', '<<TEST_CSV_SPACE>>');
...
script = script.replace("<<TEST_CSV_SPACE>>", str(test_csv).replace("\\", "/"))

matlab_file = WORK / "list_props.m"
matlab_file.write_text(script, encoding="utf-8", errors="replace")

print("Listing Interpolation properties...")
