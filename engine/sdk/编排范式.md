...
> **已删除字段**：`type`（原 producer/standard 已废弃，所有角色平等，靠 edges 连接）
...
| 已删除字段 | 原因 |
|---|---|
| `type` | producer/standard 已废弃，所有角色平等，校验角色需显式定义 |
...
### 3.5 校验角色显式定义（替代原 producer 自动展开）

原 `type: producer` 自动展开校验角色的机制已删除。需要校验的角色必须在 app.yaml 中**显式定义**独立的校验角色：
...
roles:
  ...
  需求接收者校验:                  # 校验角色（名称自定义）
    ...
    inputs:
      - 需求文档: 00-需求描述.md
    outputs:
      - 校验报告: outputs/需求接收者-validation.json, type=process

edges:
  - 需求接收者 → 需求接收者校验
  - 需求接收者校验 → 下游角色 when: result.verdict == "confirmed"
  - 需求接收者校验 → 需求接收者 when: result.verdict == "loop"   # 业务校验失败回退
```

设计原则：
- **所有角色平等**：无 producer/standard 区分，靠 edges 自由组合
- **显式即清晰**：校验角色及其边关系全部在 app.yaml 中可见，无编译期隐藏行为
- **命名自由**：校验角色名由开发者自定义，不强制格式
...
| 功能 | 触发条件 | 行为 |
|---|---|---|
| **fail 边生成** | 所有角色 | 自动生成 backward 边回到角色自身（Gate FAIL 时触发，格式错误只需修正重做） |
...
编译器全自动：
  fail 边自动生成 / carries 推导 / input_groups 计算 /
  verdicts 同步 / 骨架生成
```
