...
edges:               # 路由编排（唯一权威源）
  ...
  - A → B when: result.verdict == "xxx" max_executions: 5   # 声明边级执行上限
  - A → 完成
```
...
```
执行STEP（confirmed）→ 校验STEP（confirmed）→ 下游角色
                                     ↓ Gate FAIL
                                回退到校验STEP自身（系统自动 fail 边）
```

编译器自动行为：

- **校验角色名**：`{原角色名}（校验）`，如 `需求接收者` → `需求接收者（校验）`
- **校验角色 verdict**：继承 producer 的条件路由 verdict（通过边重定向）。producer 无条件出边自动转换为校验者的 confirmed 边。Gate FAIL 时引擎自动走 fail 边，校验角色不输出 fail
- **校验角色 inputs**：自动继承执行角色的 outputs
- **校验角色 outputs**：自动生成 `{原角色名}-validation.json`
- **向后兼容**：如果开发者在 roles 中显式定义了 `{角色名}（校验）` 角色，编译器使用用户定义的数据（skill/inputs/outputs），仅追加元数据标记 `_is_validator`。推荐使用自动展开，不显式定义校验角色
...
edges:
  ...
  # 注意：fail 边由编译器自动生成，无需在 edges 中显式声明
  # 需求接收者（校验）→ 需求接收者（系统自动 fail 边，Gate FAIL 时触发）
...
> input_groups 仅通过显式 `[A,B,C]→D` 语法生成，编译器不做 BFS 隐式推断。独立边各自生成 OR 组。
...
if input_groups:
    if from_step not in any group:   # 独立来源 → 直接放行
        pass
    elif any(group ⊆ completed):     # 任一组全完成 → 放行
        pass
    else:                             # 等待
        continue
```

> `completed` 是持久完成记录，整个执行期间保留，`_global_converge` 从此读取 JOIN 判断源。
...
| 功能 | 触发条件 | 行为 |
|---|---|---|
| **fail 边生成** | 所有角色 | 自动生成 backward 边回到角色自身（Gate 格式错误只需修正重做），不设 max_executions。app.yaml 中显式声明的 fail 边使用 max_executions 默认值 3 |
| **carries 推导** | 所有边 | normal confirmed 边→gate result；normal custom-verdict 边→gate result + 源角色 process 类型产出物；backward→源产出+gate+用户反馈（仅 manual 角色）+target 自身产出 |
| **input_groups 计算** | 显式 `[A,B,C]→D` | `[A,B,C]→D` 记录为 AND 组；独立边各自为 OR 组 |
...
> **status 字段处理（v7.0 信息性补充）**：v7.0 起，`status` 字段不再由编译器写入 schema.json 的 required 约束中。role-executor 无需显式输出 `status` 字段——它由 Hook②（post_execute 钩子）自动推导并注入到 STATE.json 的 step_status 中。此变化不改变编译器核心行为，仅影响 schema.json 骨架生成时不再将 status 列为 required 字段。详见 SDK_SPEC.md §5.1。
