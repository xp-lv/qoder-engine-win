...
STATE.json 中的持久记录。记录"哪些 step 已完成"，作为 `_global_converge` JOIN 判断的权威源。

**持久保留**：整个执行期间不清除。与 `pending_routes`（瞬态路由信号）分离——advance 同时写入两者，路由完成后只清 `pending_routes`，`completed` 保留供后续 JOIN 判断。
...
```
Phase 1: dispatch
  ├─ 有 pending_dispatches 缓存 → 消费 completed → 直接执行
  └─ 无缓存 → 从 pending_routes 路由 → _global_converge(读 completed) → set executing
...
advance 做的事：step 从 executing 变成 completed，同时写入 pending_routes，记录 verdict。
...
### 5.4 completed 持久保留 / pending_routes 瞬态路由信号

v4.1 将完成记录（`completed`）与路由信号（`pending_routes`）分离：

- **`completed`**：持久记录，advance 写入，整个执行期间保留。`_global_converge` 读此字段做 JOIN 判断。
- **`pending_routes`**：瞬态信号，advance 同步写入，路由完成后清空。`phase_dispatch` 冷路径读此字段触发路由。

```
A 完成 → completed = {A}, pending_routes = {A}
       → dispatch 路由：从 pending_routes 路由 → 找到 B
       → 清空 pending_routes → pending_routes = {}
       → completed 仍保留 A → completed = {A}
       → B 完成 → completed = {A, B}, pending_routes = {B}
       → dispatch 路由 → 清空 pending_routes
       → completed = {A, B}（JOIN 判断可用）
```
...
| `completed` | v4.1: 持久完成记录（JOIN 判断权威源）。整个执行期间保留，advance 写入 |
| `pending_routes` | v4.1: 瞬态路由信号。advance 同步写入，路由完成后清空。phase_dispatch 冷路径读此字段 |
| `edge_counts` | 每条有 max_executions 的边的执行计数。key = `"步骤ID.verdict"` |
| `terminal_state` | 非空 = 工作流结束（completed） |
| `pending_dispatches` | Phase 2/3 产出的下一步 dispatch 缓存，Phase 1 消费。追加写入，非覆盖 |
