...
### 2.4 Completed 与 Pending Routes

引擎 v4.1 采用双结构管理完成状态：

- **`completed`**（持久完成记录）：记录已完成的 step 及其 verdict/role/dispatch_id。`_global_converge` 读取此集合做 JOIN 判断（input_groups 是否满足）。整个执行期间保留，不被清除。
- **`pending_routes`**（瞬态路由信号）：advance 时写入，记录刚完成的 step 作为路由触发信号。router 消费此信号进行路由，路由完成后由 `_clear_pending_routes` 清空。
...
```json
"input_groups": [
  ["A", "B", "C"],   // 组1：A∧B∧C 全部在 completed 中 → 满足
  ["E"]               // 组2：E 在 completed 中 → 满足
]
```
...
```
Phase 1: dispatch
  ├─ 有 pending_dispatches 缓存 → 消费缓存 → 直接执行
  └─ 无缓存 → 调 router（消费 pending_routes 路由信号）→ _global_converge（检查 completed）→ set executing

主 Agent 执行 Task(role-executor)
  → role-executor 返回值中包含 verdict（通过 --verdict 参数传递给 step.py --submit）

Phase 2: post_execute
  ├─ gate PASS → 读 verdict（从 role-executor 返回值，非产出物文件）→ advance(写入 completed + pending_routes + verdict)
...
> **completed 与 pending_routes 的角色**：Phase 1 中 router 消费 pending_routes 路由信号进行路由，_global_converge 从 completed 读取 JOIN 判断源。Phase 2 中 advance 同时写入 completed（持久记录）和 pending_routes（瞬态信号）。completed 整个执行期间保留，pending_routes 路由后清空。
...
> **state_txn 原子写入（v6.0）**：Phase 1 和 Phase 2 中所有 STATE.json 的读写操作均通过 `state_io.state_txn` 上下文管理器完成原子 RMW（Read-Modify-Write），确保并发场景下的状态一致性。step_status 与 active_dispatches 在同一事务内原子写入。详见 §5.10。

> **status 字段委托 Hook②（v7.0）**：v7.0 起，step_status 中的 `status` 字段不再由 role-executor 显式写入，而是由 Hook②（post_execute 钩子）根据执行结果自动推导并注入（executing / awaiting_confirmation）。role-executor 无需关心 status 字段的写入。

> **Phase B _required_files 校验**：post_execute 阶段在 Gate 校验之外，还执行 Phase B 校验——读取 schema.json 的 `_required_files` 扩展字段，检查角色执行所需的额外必需文件是否完整（详见 §8）。缺失文件等同于 Gate FAIL，触发 fail 边回退。
...
advance 做的事：step 从 step_status 中移除，写入 completed（持久完成记录）+ pending_routes（瞬态路由信号），记录 verdict。
...
→ [["A","B"]] → A 和 B 都在 completed 中？ → 是 → 放行 D
                                           → 否 → 过滤掉 D → 返回 wait
```

### 5.4 pending_routes 路由后清空

引擎 v4.1 采用双结构管理完成状态，pending_routes 是瞬态信号，路由后清空；completed 是持久记录，整个执行期间保留：

```
A 完成 → completed = {A}, pending_routes = {A}
       → dispatch B（router 消费 pending_routes 路由信号）
       → 清空 pending_routes → pending_routes = {}
       → completed 仍保留 {A}（供后续 JOIN 判断）
       → B 完成 → completed = {A, B}, pending_routes = {B}
```

> **completed 是持久的**：completed 集合在整个执行期间保留，_global_converge 全程从此读取 JOIN 判断源。**pending_routes 是瞬态的**：路由完成后由 `_clear_pending_routes` 清空，仅用于触发路由。
...
引擎 v6.0 起，所有 STATE.json 的读写通过 `state_io.state_txn` 上下文管理器完成原子 RMW（Read-Modify-Write）操作，确保在并发场景下（如并行分支同时推进）STATE.json 的一致性：

```
with state_txn(workspace_id) as state:
    state["step_status"][step_id] = {...}
    state["active_dispatches"][step_id] = {...}  # 同一事务内原子写入
```

state_txn 保证：
- 读取当前 STATE.json → 修改 → 原子写回（文件级锁）
- step_status 和 active_dispatches 在同一事务内写入，确保状态一致性
- 事务期间其他读操作获取的是事务前的快照

### 5.11 引擎错误诊断

当 dispatch 无产出（候选为空）时，引擎通过 `_diagnose_wait_reason` 将模糊的等待原因转化为具体诊断：

| 诊断场景 | 检测条件 | 诊断结果 |
|---|---|---|
| JOIN 等待 | 候选 step 的 input_groups 未全部满足 | 等待同步汇入的前置步骤完成 |
| 路由信号无匹配 | pending_routes 中的 step 无对应 transition | 路由配置缺失或已完成 |
| STATE 不一致 | step_status 与 completed 状态矛盾 | 状态不一致，需人工介入 |

当 `is_error=True` 时，引擎将诊断信息写入 STATE.json 的 `engine_error` 字段（结构见 §6），标记为引擎级错误状态。advance 成功时自动清除 engine_error。

### 5.12 快照机制

引擎 v6.0 起，在每次 advance 成功后自动保存路由快照到 `snapshots/{step}.json`：

```
advance 成功 → 保存 STATE.json 快照到 {workspace}/snapshots/{completed_step}.json
```

快照记录了 advance 后的完整 STATE.json 状态，用于：
- 执行轨迹回溯
- 调试时定位状态分歧点
- `--list-workspaces` 命令通过 `has_snapshots` 标志位指示工作区是否有快照（详见 §7）
...
```json
{
  ...
  "completed": {
    "步骤ID": {"verdict": "confirmed", "role": "角色名", "id": "ckpt_xxx", "created_at": "ISO时间"}
  },
  "pending_routes": {
...
| `completed` | _global_converge JOIN 判断的权威源，记录已完成的 step 及其 verdict/role/dispatch_id/created_at（advance 时间戳）。整个执行期间保留，不被清除 |
| `pending_routes` | advance 时写入的路由信号，记录刚完成的 step。router 消费此信号进行路由，路由完成后清空 |
...
| `active_dispatches` | v6.0 活跃 dispatch 缓存，key=step ID，value=dispatch 指令。与 step_status 在同一 state_txn 内原子写入 |
| `cached_branch_results` | 分支结果缓存，记录已完成分支的产出供 JOIN 判断 |
| `engine_error` | v7.1 引擎错误标志位。null=无错误；非空 dict 结构为 {reason, timestamp, last_good_step}，advance 成功时自动清除 |
| `dispatch_log` | v7.2 分发轮次日志，结构 [{round, steps, parallel, from_steps, timestamp}]，记录每次 dispatch 的轮次信息 |
| `project_id` | 项目目录名（与 workspace_id 并存的遗留字段） |
...
| `--verdict` | 否 | 角色 verdict 值，**从 role-executor 返回值读取**。路由决策的核心输入——未传入时所有角色按 confirmed 处理（条件路由失效）。推荐传入以启用条件路由 |
...
v7.2 新增命令。从 `runtime/workspaces/index.json` 读取工作区注册表，列出所有工作区的信息：

| 字段 | 说明 |
|---|---|
| `app` | 工作区关联的应用名 |
| `status` | 工作区状态（active/idle） |
| `completed_count` | 已完成步骤数 |
| `terminal_state` | 终态（null=未结束，completed=已完成） |
| `schema_version` | STATE.json schema 版本 |
| `has_snapshots` | 是否有快照（详见 §5.12） |
| `last_active_at` | 最后活跃时间 |
| `is_active` | 是否当前活跃工作区 |

### directive 自动生成

step.py 在所有输出 JSON 中自动注入 `directive` 文本字段（通过 `_gen_directive` 函数）。该字段根据 action/next/status 生成主 Agent 应执行的指令文本：

| action | directive 示例 |
|---|---|
| `delegate` | "发起 N 个 Task(role-executor)" + 具体角色信息 |
| `wait` | "BLOCKING：等待中，原因：..." |
| `complete` | "工作流已完成" |
| `confirm` | "请用户确认步骤 XXX" |
| `error` | "BLOCKING：引擎错误，请检查..." |

主 Agent 只需读取 `directive` 字段的字面值并执行，无需自行解析 action 语义。
...
> **_required_files 扩展字段（v6.0）**：schema.json 支持顶层 `_required_files` 数组，声明角色正常执行所需的额外必需文件列表：

```json
{
  "$schema": "http://json-schema.org/draft-07/schema#",
  "_required_files": [
    {"name": "上游产出", "path": "outputs/xxx.json", "type": "deliverable"}
  ],
  "type": "object",
  "required": ["result"],
  "properties": { ... }
...
orchestrator 在 post_execute Phase B 读取此字段，检查每个声明的文件是否存在且非空。缺失文件等同于 Gate FAIL，触发 fail 边回退。路径中包含 `{` 的模板路径（如 `{WORKSPACE_ROOT}/...`）跳过检查。

> **校验职责分工**：`_required_files` 校验由 orchestrator Phase B 执行（产出物完整性），Gate（§9）负责格式校验。两者独立运行，任一失败即触发回退。
...
| 二进制文件短路 | 扩展名匹配 BINARY_EXTENSIONS（.png/.jpg/.jpeg/.gif/.bmp/.ico/.webp/.svg/.woff/.woff2/.ttf/.eot/.pdf/.zip/.tar/.gz）的文件，物理检查（存在+非空）通过即 PASS，跳过 schema 校验 |
| 目录类型产出物 | type 为目录的产出物，检查目录存在且非空即 PASS，跳过内容校验 |
| UTF-8 解码失败 | 无法解码为 UTF-8 的非二进制扩展名文件，物理检查通过即 PASS |
| 非 JSON 文本包装 | 非 JSON 文本文件（如 .md/.txt）包装为 `{"_raw_text": ...}` 后仅做最小长度检查，跳过 schema 校验 |
